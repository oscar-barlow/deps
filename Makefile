...
repos:
	...
	cat repositories.txt | sed "s/\$$(lsb_release -cs)/$$(grep VERSION_CODENAME /etc/os-release | cut -d= -f2)/g" | while read line; do grep -Fxq "$$line" /etc/apt/sources.list.d/custom-repos.list 2>/dev/null || echo "$$line" | tee -a /etc/apt/sources.list.d/custom-repos.list; done
